...
function TMJ = Render(TMJ,X)
    if isempty(TMJ.RenderObj)
        ...
        TMJ.RenderObj.Spring = DrawSpring(TMJ, [0, X(1)], [0, X(3)+bH], 0);
...
%         text(x1a-0.04*TMJ.L, y1a+0.12*TMJ.L, '{\itm, L, I}','FontSize',24,'FontWeight','bold','HorizontalAlignment','Right');
    ...
    function [ hObj ] = DrawSpring(TMJ, PosS, PosE, Z)
        NumTurns=5;
        ...
        if ~isfield(TMJ.RenderObj,'Spring')
            hObj = ...
                plot3(Coords(1,:), Coords(2,:), Z, 'Color', [0, 0, 0], 'LineWidth', 2*TMJ.LineWidth);
        else
            set(TMJ.RenderObj.Spring,'XData',Coords(1,:));
            set(TMJ.RenderObj.Spring,'YData',Coords(2,:));
        end
    end
